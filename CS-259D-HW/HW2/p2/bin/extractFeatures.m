...
featureStr{35} = 'beginning to mid stroke pressure variation';
featureStr{36} = 'median area variation';
...
for i = 1:Nstrokes
    ...
    % pressure variation from beginning to mid-stroke
    featMat(i, 35) = featMat(i, 30) - x_stroke(downInd(i), col_press)
    
    % covered area in the middle of the stroke
    featMat(i, 31) = median( x_stroke(floor(npoints/2):ceil(npoints/2) ,col_area) );

    % median variance in covered area over the stroke
    list = []
    for j = downInd(i):(downInd(i+1)-1)
	for k = j:(downInd(i+1)-1)
	   list = [list, abs(x_stroke(j, col_area)-x_stroke(k, col_area))]
	end
    end
    featMat(i, 36) = median(list)
