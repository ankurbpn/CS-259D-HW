...
def find_best_threshold():
	M, NUM_SEQUENCES, LENGTH_SEQUENCES, TOTAL_TRAINING_SEQUENCES,TRAINING_SEQ, SCOPE, THRESHOLD, N = get_global_vars()
	m2 = np.matrix(np.loadtxt(open("results.csv", 'rb'), delimiter = ','))
	...
	with open("threshold_finding.txt", "w") as myfile:
...
def final_analysis():	
	false_vals = np.matrix(np.loadtxt(open("threshold_finding.csv", 'rb'), delimiter = ","))
